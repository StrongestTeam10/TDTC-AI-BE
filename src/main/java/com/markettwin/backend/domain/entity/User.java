 ...
 * USRUSRS01M - 사용자
 *
 * 회원가입 관리자 승인(approvalStatus) + 비밀번호 찾기(updatePassword) 추가.
 * 관리자 회원 권한 변경 기능(UserAdminService)에서 rulesCode를
...
public class User {
    ...
    // FE 회원가입 화면의 동의 항목(서비스 이용약관/개인정보 수집·이용은
    ...
    // 추가(게시판): 담당 시장 코드(comcode01m MKT 도메인). 게시판 목록에서
    ...
    // (회원가입 관리자 승인): comcode01m(code_cob='APR') 코드값.
    ...
    // (비밀번호 찾기): 재설정 시 비밀번호 + 변경 이력(시각/IP)을 함께 갱신
