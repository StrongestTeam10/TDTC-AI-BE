...
/**
 * 실측 상태에서 출발한 예측 시뮬레이션 요청. SIM PredictRequest와 1:1 매칭.
 *
 * 실제 관측된 인원 배치를 초기 상태로 삼아, 매대(오브젝트) 매력도 기반 자연스러운
 * 이동과 게이트를 통한 신규 유입만으로 "인구가 몰렸을 때" 위험도가 어떻게
 * 전개되는지를 본다.
 *
 * 오브젝트 배치/통로정책도 Before가 받는다. 시장 구조 등록에서
 ...
 *
 * events(화재). FE는 이전부터 Before 화면에서도 이벤트를
...
public record PredictRequestDto(
        ...
        // (: inflowPerStep에서 totalInflow로 변경 — 고정 인원/스텝 대신
        ...
        // CCTV 관측 초기배치. BE가 현재 프레임 픽셀좌표를 구역 폴리곤에
