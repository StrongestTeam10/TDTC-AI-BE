...
-- V1 — 기준 스키마 (baseline)
--
-- 2026-08-13: Flyway 도입. 기존 schema-init.sql을 그대로 옮겨온 파일이며,
-- 이 시점의 전체 스키마를 나타낸다.
--
-- ⚠️ 이 파일은 "이미 돌고 있는 DB"에서는 실행되지 않는다.
--    application.yml의 baseline-on-migrate=true / baseline-version=1 설정에 따라,
--    테이블이 이미 있는 DB(운영 RDS·개발 DB)는 이 V1을 적용된 것으로 표시만 하고
--    건너뛴다. 빈 DB에서만 실제로 실행된다.
--
-- ⚠️ 이 파일은 앞으로 절대 수정하지 않는다. 적용이 끝난 마이그레이션의 내용을
--    바꾸면 체크섬이 달라져 Flyway가 기동을 거부한다.
--    스키마를 바꾸려면 V2, V3... 새 파일을 추가할 것.
--
-- 시드 데이터(comcode-seed.sql, seed-market-data.sql)는 Flyway가 관리하지 않는다.
-- 이유는 README의 "DB 마이그레이션" 절 참고.
-- =========================================================================
